...
WHERE s.idx_scan < 10
  ...
  AND pg_relation_size(s.indexrelid) > 8192  -- larger than one page
...
#Invalid Indexes
SQL22="SELECT n.nspname AS schema_name,
...
SQL23="SELECT n.nspname AS schema_name,
       c.relname AS table_name,
...
SQL24="SELECT datname AS database_name,
       blks_hit,
       blks_read,
       round((100.0 * blks_hit / nullif(blks_hit + blks_read, 0))::numeric, 2) AS cache_hit_ratio
...
SQL25="SELECT schemaname AS schema_name,
       sequencename AS sequence_name,
       data_type,
       last_value,
       max_value,
       round((100.0 * last_value / max_value)::numeric, 2) AS pct_used
FROM pg_sequences
WHERE last_value IS NOT NULL
  AND max_value > 0
  AND (100.0 * last_value / max_value) > 30
ORDER BY pct_used DESC
LIMIT 10;"

#Extensions Installed
SQL26="SELECT extname AS extension_name,
...
echo "<br>" >> $html
echo "Database Name: $DBNAME" >> $html
echo "<br>" >> $html
...
echo "<br>" >> $html
echo "Current Total Connections: " >>$html
...
aws rds describe-db-instances --db-instance-identifier $RDSNAME | grep 'DBInstanceClass\|"AllocatedStorage"\|MaxAllocatedStorage\|Public\|MonitoringInterval\|MultiAZ\|StorageType\|BackupRetentionPeriod'|sed "s/\"//g"|sed "s/\,//g"| sed "s/\PubliclyAccessible/<br>Publicly Accessible/g"| sed "s/\MonitoringInterval/<br>EM Monitoring Interval/g" | sed "s/\MultiAZ/<br>Multi AZ Enabled?/g" | sed "s/MaxAllocatedStorage/<br>Storage Autoscaling Limit (GB)/g" | sed "s/\AllocatedStorage/<br>Current Storage (GB)/g" |  sed "s/\DBInstanceClass/<br>DB Instance Class/g" | sed "s/\BackupRetentionPeriod/<br>Backup Retention Period/g" | sed "s/\StorageType/<br>Storage Type/g" |  sed "s/\ B//g"|sed "s/\gp2/GP2/g" | sed '1s/^<br>//' >>$html
echo "<br>" >> $html
echo "<br>" >> $html

echo "  [1/8] Collecting instance details and health score..."
...
if [[ "$SCORE18" != "all"  ]]; then i18=$((i17+5)); else i18=$i17; fi
echo " <tr>
...
i20=$i19
if [ "$DBTYPE" == "postgres" ] && [ "$RATIOSB" -gt 23 ]; then 
    i20=$((i19+5))
elif [ "$DBTYPE" == "aurora-postgresql" ] && [ "$RATIOSB" -gt 65 ]; then
    i20=$((i19+5))
fi
echo "<tr>
...
echo "<font face="verdana" color="#ff6600">Infrastructure Metrics (CloudWatch): </font>" >>$html
echo "<br>" >> $html

# Function to get CloudWatch metric average
get_cw_metric() {
  local metric_name=$1
  local days=$2
  local end_time=$(date -u '+%Y-%m-%dT%H:%M:%SZ')
  local start_time=""
  # Try GNU date first, fall back to BSD (Mac) date
  start_time=$(date -u -d "$days days ago" '+%Y-%m-%dT%H:%M:%SZ' 2>/dev/null) || \
  start_time=$(date -u -v-${days}d '+%Y-%m-%dT%H:%M:%SZ' 2>/dev/null)
  local period=$((days * 86400))
...
# Collect metrics for 1 day, 15 days, 35 days
CPU_1D=$(get_cw_metric "CPUUtilization" 1)
CPU_15D=$(get_cw_metric "CPUUtilization" 15)
CPU_35D=$(get_cw_metric "CPUUtilization" 35)

MEM_1D=$(get_cw_metric "FreeableMemory" 1)
MEM_15D=$(get_cw_metric "FreeableMemory" 15)
MEM_35D=$(get_cw_metric "FreeableMemory" 35)

RIOPS_1D=$(get_cw_metric "ReadIOPS" 1)
RIOPS_15D=$(get_cw_metric "ReadIOPS" 15)
RIOPS_35D=$(get_cw_metric "ReadIOPS" 35)

WIOPS_1D=$(get_cw_metric "WriteIOPS" 1)
WIOPS_15D=$(get_cw_metric "WriteIOPS" 15)
WIOPS_35D=$(get_cw_metric "WriteIOPS" 35)

CONN_1D=$(get_cw_metric "DatabaseConnections" 1)
CONN_15D=$(get_cw_metric "DatabaseConnections" 15)
CONN_35D=$(get_cw_metric "DatabaseConnections" 35)

# Format values for display using awk only (portable across Mac/Linux)
fmt_pct() { echo "$1" | awk '{if ($1+0 == $1) printf "%.2f %%", $1; else print "N/A"}'; }
fmt_mb() { echo "$1" | awk '{if ($1+0 == $1) printf "%.0f MB", $1/1048576; else print "N/A"}'; }
fmt_num() { echo "$1" | awk '{if ($1+0 == $1) printf "%.0f", $1; else print "N/A"}'; }

echo "<table border=1 cellpadding=3 cellspacing=0>
  <tr>
    <td><font face="verdana" color="#000000"><b>Metric</b></font></td>
    <td><font face="verdana" color="#000000"><b>Last 1 Day (Avg)</b></font></td>
    <td><font face="verdana" color="#000000"><b>Last 15 Days (Avg)</b></font></td>
    <td><font face="verdana" color="#000000"><b>Last 35 Days (Avg)</b></font></td>
  </tr>
  <tr>
    <td>CPU Utilization (%)</td>
    <td>$(fmt_pct "$CPU_1D")</td>
    <td>$(fmt_pct "$CPU_15D")</td>
    <td>$(fmt_pct "$CPU_35D")</td>
  </tr>
  <tr>
    <td>Freeable Memory (MB)</td>
    <td>$(fmt_mb "$MEM_1D")</td>
    <td>$(fmt_mb "$MEM_15D")</td>
    <td>$(fmt_mb "$MEM_35D")</td>
  </tr>
  <tr>
    <td>Read IOPS</td>
    <td>$(fmt_num "$RIOPS_1D")</td>
    <td>$(fmt_num "$RIOPS_15D")</td>
    <td>$(fmt_num "$RIOPS_35D")</td>
  </tr>
  <tr>
    <td>Write IOPS</td>
    <td>$(fmt_num "$WIOPS_1D")</td>
    <td>$(fmt_num "$WIOPS_15D")</td>
    <td>$(fmt_num "$WIOPS_35D")</td>
  </tr>
  <tr>
    <td>Database Connections</td>
    <td>$(fmt_num "$CONN_1D")</td>
    <td>$(fmt_num "$CONN_15D")</td>
    <td>$(fmt_num "$CONN_35D")</td>
  </tr>
</table>" >> $html
echo "<font face="verdana" color="#0099cc">Note: These are average values from CloudWatch metrics. For CPU spikes and memory pressure events, check CloudWatch dashboards or Performance Insights for detailed time-series data.</font>" >> $html

echo "<br>" >> $html
echo "<br>" >> $html

echo "<font face="verdana" color="#ff6600">Extensions Installed: </font>" >>$html
echo "<br>" >> $html
echo "`PGPASSWORD=$MYPASS $PSQLCL --html -c "SET statement_timeout='60s' ; SET idle_in_transaction_session_timeout='60s';
$SQL26
"|sed '$d'|sed '$d'|grep -v '<p>SET</p>' ` " >>$html
echo "<font face="verdana" color="#0099cc">Note: Consider installing <a href=\"https://www.postgresql.org/docs/current/pgstatstatements.html\" target=\"_blank\">pg_stat_statements</a> for query performance analysis, <a href=\"https://www.postgresql.org/docs/current/pgbuffercache.html\" target=\"_blank\">pg_buffercache</a> for buffer cache analysis, and <a href=\"https://www.postgresql.org/docs/current/pgprewarm.html\" target=\"_blank\">pg_prewarm</a> for buffer cache warming after restart.</font>" >> $html
...
echo "<br>" >> $html
echo "<br>" >> $html
echo "  [2/8] Analyzing database sizes and tables..."
...
echo "<br>" >> $html
echo "<br>" >> $html
echo "<font face="verdana" color="#ff6600">Tables Without Primary Key: </font>" >>$html
echo "<br>" >> $html
echo "`PGPASSWORD=$MYPASS $PSQLCL --html -c "SET statement_timeout='60s' ; SET idle_in_transaction_session_timeout='60s';
$SQL23
"|sed '$d'|sed '$d'|grep -v '<p>SET</p>' ` " >>$html
echo "<font face="verdana" color="#0099cc">Note: Tables without a primary key can cause issues with logical replication, and may result in full table scans for UPDATE/DELETE operations. Consider adding primary keys to these tables.</font>" >> $html

echo "<br>" >> $html
echo "<br>" >> $html
echo "  [3/8] Checking indexes..."
...
echo "<br>" >> $html
echo "<br>" >> $html
echo "<font face="verdana" color="#ff6600">Rarely Used Indexes: </font>" >>$html
echo "<br>" >> $html
echo "`PGPASSWORD=$MYPASS $PSQLCL --html -c "SET statement_timeout='60s' ; SET idle_in_transaction_session_timeout='60s';
$SQL6
"|sed '$d'|sed '$d'|grep -v '<p>SET</p>' ` " >>$html
echo "<font face="verdana" color="#0099cc">Note: For better write performance, saving write IOs, saving storage, look at the index definitions and consider dropping rarely used indexes." >> $html

echo "<br>" >> $html
echo "<br>" >> $html
echo "<font face="verdana" color="#ff6600">Invalid Indexes: </font>" >>$html
echo "<br>" >> $html
echo "`PGPASSWORD=$MYPASS $PSQLCL --html -c "SET statement_timeout='60s' ; SET idle_in_transaction_session_timeout='60s';
$SQL22
"|sed '$d'|sed '$d'|grep -v '<p>SET</p>' ` " >>$html
echo "<font face="verdana" color="#0099cc">Note: Invalid indexes are left behind from failed CREATE INDEX CONCURRENTLY operations. They consume storage but are never used for queries. Drop and recreate them.</font>" >> $html
...
echo "<br>" >> $html
echo "<br>" >> $html
echo "<font face="verdana" color="#ff6600">Sequences Nearing Exhaustion (>30% used): </font>" >>$html
echo "<br>" >> $html
echo "`PGPASSWORD=$MYPASS $PSQLCL --html -c "SET statement_timeout='60s' ; SET idle_in_transaction_session_timeout='60s';
$SQL25
"|sed '$d'|sed '$d'|grep -v '<p>SET</p>' ` " >>$html
echo "<font face="verdana" color="#0099cc">Note: Sequences using int4 (integer) data type have a max value of ~2.1 billion. If a sequence is approaching exhaustion, consider altering it to use bigint (int8) to avoid insert failures.</font>" >> $html

echo "<br>" >> $html
echo "<br>" >> $html
echo "  [4/8] Analyzing bloat and vacuum stats..."
...
echo "  [5/8] Collecting table activity and vacuum parameters..."
...
echo "<br>" >> $html
echo "<br>" >> $html
echo "  [6/8] Reviewing PostgreSQL parameters..."
...
echo "<br>" >> $html
echo "  [7/8] Analyzing IO patterns..."
...
echo "<br>" >> $html
echo "<br>" >> $html
echo "<font face="verdana" color="#ff6600">Database Cache Hit Ratio: </font>" >>$html
echo "<br>" >> $html
echo "`PGPASSWORD=$MYPASS $PSQLCL --html -c "SET statement_timeout='60s' ; SET idle_in_transaction_session_timeout='60s';
$SQL24
"|sed '$d'|sed '$d'|grep -v '<p>SET</p>' ` " >>$html
echo "<font face="verdana" color="#0099cc">Note: A cache hit ratio below 99% typically indicates that shared_buffers is undersized for the workload, or the working dataset exceeds available memory. Monitor CloudWatch metric 'BufferCacheHitRatio' and consider increasing shared_buffers.</font>" >> $html


if
PGPASSWORD=$MYPASS $PSQLCL -t -c "select extname FROM pg_extension where extname='pg_stat_statements'" 2>/dev/null | grep -qw pg_stat_statements; then
echo "  [8/8] Analyzing pg_stat_statements (top queries)..."
echo "<br>" >> $html
echo "<br>" >> $html
